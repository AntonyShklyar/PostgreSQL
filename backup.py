...
def main():
    ...
    def paths(a, domain, codname, codid):
        ...
        for k, (i, j) in enumerate(s.items(), 1):
            try:
                temp = subprocess.check_output("ls -d /mnt/dbbackup/" + i + "/*/ | grep $(uname -n)", shell = True).decode("utf-8")
                temp = temp.split("\n")
                temp.remove('')
    ...
    def replication(*args):
        ...
        for k,i in enumerate(path, 1):
            ...
            if exit_code == 0:
                with open("/var/log/backupdb.log","a+") as stdout:
                    stdout.write(str(datetime.now().strftime("%Y%m%d-%H%M%S")) + ' Replicating a PostgreSQL database backup to another data center storage Successfully ' + '\n')
	#For a server that has '01' in its name, walfiles are not backed up or copied.
                ...
                with open("/var/log/backupdb.log", "a+") as stdout:
                    stdout.write(str(datetime.now().strftime("%Y%m%d-%H%M%S")) + ' Replicating a PostgreSQL database backup to another data center storage Unsuccessfully ' + '\n')
                exit()
        '''
        Input data:
        path - data type is a list - Storage mount point
        copy - data type is string - PostgreSQL database backup name
        copywal - data type is string - PostgreSQL WAL backup name (for a server in a cluster)
        Output data:
        The returned data type is a number.
        If 0 is returned, services are available.
        Otherwise, the program terminates.
	...
	(reasons for creating an incorrect copy: manual shutdown of the program, reboot/shutdown/freeze of the storage server/database server, problems with network availability)
        '''
        if os.path.exists(path + 'temp/' + 'base.tar.gz'):
		...
		#Counting the time it takes to complete a backup operation.The execution time is stored in a variable "TIME"
                ...
                id = subprocess.check_output("cat /etc/passwd | grep postgres | awk -F ':' '{print $3}'", shell = True).decode("utf-8")
                id = id.split("\n")[0]
                ...
                if process == 0:
                    with open("/var/log/backupdb.log","a+") as stdout: 
                        ...
                        b = subprocess.Popen('echo backupdb-$(uname -n)-$(date +"%Y%m%d-%H%M%S").tar.gz', shell = True, stdout = subprocess.PIPE)
                        copy = subprocess.check_output(('xargs', 'echo'), stdin = b.stdout).decode("utf-8")
                        copy = copy.split("\n")[0]
    ...
    def sync(path1, path2, a, b, c, d, processors = 2):
        ...
        def copy(path1, path2, a):
            '''Copying a backup to another data center storage'''
            for i in a:
                exit_code = subprocess.call(['cp', path1 + i, path2])
            '''
            Input data:
            path1 - data type is a string - Primary storage mount point
            path2 - data type is a string - Secondary storage mount point
            a - data type is a list - Primary list of backups
            Output data:
            The function returns 0
            '''
        def submassive(a, processors):
            '''List formation of backups for each processor'''
            ...
            Input data:
            a - data type is a list - Primary list of backups
            processors - data type is a number - count of processors in server
            Output data:
            each_chunk - data type is a list - List of backups for 1 processor
            '''
        def syncronize(path1, path2, m):
            ...
            path1 - data type is a string - Primary storage mount point
            path2 - data type is a string - Secondary storage mount point
            m - data type is a list - List of lists backups for each processor
            Output data:
            The function returns 0
            '''
        if d == 0:
        ...
        for n, k in enumerate(b, 1):
            if k not in a:
                ...
                if n == len(temp) and len(s) ! = 0:
                    ...
                    if len(s) == 1:
                        ...
                        m = list((submassive(s, processors)))
                        syncronize(path1, path2, m)
                    return 0
            else:
                if n == len(temp) and len(s) ! = 0:
            ...
            path2 - data type is a string - Secondary storage mount point
            a - data type is a list - Primary list of backups
            b - data type is a list - Secondary list of backups
            c - data type is a number - Count of backups in Primary list
            d - data type is a number - Count of backups in Secondary list
            processors - data type is a number - count of processors in server
            Output data:
            The function returns 0
            Otherwise, the program ends
    ...
    def search(path, i):
        ...
        def checkcopy(path, i):
            ...
            for j in path:
                ...
                if i == 0:
                    try:
                        temp1 = "ls -rI wal* "+j+" | grep tar.gz | tail -n +2"; a = subprocess.check_output(temp1,shell = True).decode("utf-8")
                        a = a.split("\n"); a.remove(''); b = len(a)
                    except subprocess.CalledProcessError:
                        a = []
                        b = 0
                    ...
                    try:
                        temp1 = "ls -t "+j+" | grep wal | tail -n +2"; a = subprocess.check_output(temp1,shell = True).decode("utf-8")
                        a = a.split("\n"); a.remove(''); b = len(a)
                    except subprocess.CalledProcessError:
                        a = []
                        b = 0
	...
	#The cycle will run until the number of backups and WAL-archives in the vaults becomes the same
        for n in range(2):
            ...
            if n == 1:
                s = max(count.items(), key = itemgetter(1))[1]
                '''
                For Python 2
                ...
                m = max(count.items(), key = itemgetter(1))[1]
                '''
                For Python 2
        ...
        Input data:
        path - data type is a list - Storage mount point
        i - data type is a number - counter
        Output data:
        The program ends
        '''
    def mounts(s, dic):
	...
	#Checking the availability of hypervisor servers with ICMP backup storages
        ...
        for i, j in l.items():
            for x in j:
                ...
                exit_code = subprocess.check_output(("sed", "-n", '1,/^---/d;s/%.*//;s/.*, //g;p;q'), stdin = ping.stdout)
